	...
	methods:{
		showDeleteConfirmationModal(){
	        this.$bvModal.msgBoxConfirm('Please confirm that you want to delete this communication.', {
	         ...
	         .then(value => {
	         	if(value){
	         		var that = this;
		           	this.$http.delete(`candidates/${this.candidate_id}/communications/${this.communication_id}`).then(function(response){
		           		that.$emit('deleted');
		           	}).catch(function(){
		...
		submit(){
			if(this.cruType=='create'){
				this.form.post(`candidates/${this.candidate_id}/communications`);
			}
			else{
				this.form.patch(`candidates/${this.candidate_id}/communications/${this.communication_id}`);
			}
		},
	},
	computed:{
