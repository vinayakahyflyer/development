<template>
<div>
	<b-modal id="user-cru-modal" ref="user-cru-modal" :title="((cruType=='create')?'Add ':'Edit ')+'User'" class="modal-top" hide-footer static no-close-on-esc no-close-on-backdrop>
     	<kr-form ref="user-form" v-slot="{flags,actions,data}" :options="formOptions" class="my-2">
	    	<user-form :data="data" :hideStatus="krAuth.user().id==user_id" @submit="!(flags.invalid || !flags.valid)?submit():''"></user-form>
...
export default{
	data(){
		...
		return{
			...
			formOptions:{
		        fields:{
		        	role_id:{default:5,rules:'required'},
		        	organisation_id:{default:this.$attrs.organisation_id,rules:'required'},
